...
import org.apache.spark.sql.functions.{ explode, udf, col, concat, lit }
...
class ContractRegistryXMLParser(val spark: SparkSession, val xmlSource: String, val charset: String = "ASCII") {
  ...
  def parse(): ContractRegistryData = {
    ...
    val flatData = rawData
      ...
      .withColumn("id", concat(col("idSmlouvy"), lit("_"), col("idVerze")))
    ...
    log.info(s"attachments schema: ${attachmentsData.schema}")
    log.info(s"contract party schema: ${contractPartyData.schema}")
    log.info(s"registry schema: ${registryData.schema}")
